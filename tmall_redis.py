...
class TmallSpider(object):
    def check_itemid(self, itemId, shop_name):
        # itemId_sha1 = hashlib.sha1(itemId.encode()).hexdigest()
        # flag = REDIS.sadd('itemId', itemId_sha1)
        item = dict(shop_name=shop_name, itemId=itemId)
        # 添加进redis集合，表示没有抓取，返回False
        flag = REDIS.sadd('item', json.dumps(item))
        if flag:
            return False
        return True
    ...
    def download_image(self, detail_url, item, shop_name):
        ...
        print(f'{itemId} crawl finished!' + '*'*100)
        ...
        for shop_name in shop_name_list:
            ...
            while not next_page:  # 是否有下一页
                ...
                for detail_url in detail_url_list[:-8]:
                    # 2.进入详情页提取数据
                    itemId = re.findall('id=(\d+)', detail_url)[0]
                    if self.check_itemid(itemId, shop_name):  # 判断是否已抓取
                        print('crawled!')
                        continue
